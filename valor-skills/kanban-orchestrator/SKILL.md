...
### RockTeam naming for this user

When the user says "RockTeam" on this machine, distinguish two teams:

- **persona RockTeam** = the original persona-based team. The main orchestrator reads each role's `SOUL.md` and spawns `delegate_task` subagents directly. Use this for quick analysis, research, architecture discussion, and second opinions where a persistent board trail is unnecessary.
- **kanban RockTeam** = the Hermes Kanban profile-agent team. Use this for formal multi-stage work that needs durable tasks, artifacts, review gates, notification subscriptions, and main-gated handoff.
- If the user explicitly names one, use that one. If they only say "RockTeam", choose based on the task class and state the choice briefly.

This is a local convention layered on top of the generic Step-0 profile discovery — the actual profile names you'll find via `hermes profile list` are still what the dispatcher routes on.
...
## Monitoring a newly created task

After `kanban_create`, do a quick liveness check before telling the user the chain is healthy:

1. Call `kanban_show(task_id)`.
2. Confirm the event stream has progressed beyond `created`; a healthy dispatched task normally shows `created → claimed → spawned`.
3. Confirm `runs[*].status` is `running` or `done`, and note `current_run_id`.
4. If a `spawned` event has `pid`, verify the worker exists with `ps -p <pid> -o pid,ppid,stat,etime,cmd`.
5. Inspect worker logs when needed. Logs are normally under `~/.hermes/kanban/logs/<task_id>.log`, and the CLI also exposes `hermes kanban log <task_id>`.

A task that is only `ready` may still be valid, but do not describe it as actively running until a worker has claimed/spawned it. If it stays ready unexpectedly, check dispatcher/gateway health rather than creating duplicate tasks.

See `references/threads-viewer-kanban-first-change-2026-05-06.md` for a concrete first-change smoke pattern using a `research → product → architect → coding → review` chain.

For this user's **kanban RockTeam**, preserve the corrected main-gate design: specialist workers only complete their own task and provide `suggested_next`; they do **not** create the next task themselves. The gate is at **chain authorization**, not necessarily at every in-chain handoff. If the user explicitly asked for Kanban / RockTeam chain / full-chain execution, a main-owned watcher may automatically create the next child task from valid `suggested_next` metadata, after structural artifact checks and idempotency guards. If the user did **not** explicitly authorize Kanban, do not start or expand a Kanban chain proactively. See `references/kanban-auto-chain-policy.md` for the corrected policy, and `references/kanban-main-gate-and-monitoring.md` for the older manual-gate context.

**Cron syntax gotcha:** when using Hermes cron for a recurring no-agent monitor, set the schedule as `every 5m`, not bare `5m`. Bare `5m` is interpreted as a one-shot "once in 5m" job and the monitor will complete after one run. After updating the job, verify `cronjob list` shows `repeat: forever`, `enabled: true`, and a future `next_run_at`.

## Notification subscriptions for programmatic tasks

When the user expects completion pings for tasks you create through tools or CLI, treat notification wiring as part of orchestration:

1. Verify current state with `hermes kanban notify-list` or `hermes kanban notify-list <task_id>`.
2. **Also inspect the task status immediately before and after subscribing** with `hermes kanban show <task_id>` / `hermes kanban runs <task_id> --json`. If the task is already `blocked`, `done`, `gave_up`, `crashed`, or `timed_out`, do not merely say "subscribed" — report the terminal reason to the user right away.
3. If there is no row for the active chat and the task is not already terminal, subscribe explicitly:
   ```bash
   hermes kanban notify-subscribe <task_id> --platform discord --chat-id <chat_id> [--thread-id <thread_id>]
   ```
4. Verify the row exists before telling the user notifications are enabled.
5. Repeat for each newly created child task; subscriptions are per task and are not inherited through `parents=[...]`.

**Valor / OpenClaw operating preference:** for this user's Kanban chains, assume every created or newly discovered Kanban task must be tracked unless they explicitly opt out. Do not wait for a complaint after a task is created. Immediately subscribe the current conversation, check `runs`, and for multi-stage chains install or update a small idempotent monitor so future child tasks are also subscribed.

Slash-command-created tasks may auto-subscribe to the source chat, but programmatic `kanban_create` / scripted create calls do not. If subscribing after a terminal event already happened, seed the subscription cursor to the current last event if you only want future events. See `references/kanban-notification-subscriptions.md` for exact commands, terminal event kinds, late-subscribe recovery, Discord target notes, and a lightweight auto-tracking pattern.

## Pitfalls

**Do an orchestrator gate before spawning the next stage.** A parent task reaching `done` only means that worker completed its assignment; it does not mean its handoff is safe to forward. Before creating the child task, inspect the metadata and any long-form artifact paths. If the handoff conflicts with product decisions, safety rules, data-retention constraints, or the requested scope, do not create the implementation/review child yet. Add a comment explaining the blocker and create a corrective task for the upstream role, linked from the flawed task. See `references/kanban-chain-gate-review-and-background-monitoring.md` for the concrete pattern.
